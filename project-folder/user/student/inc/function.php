...
if (isset($_POST["submit"])) {
	...
	if (mysqli_num_rows($sql2_u) > 0) {
		...
		$insert = mysqli_query($link, "insert into std_registration values('','$name','$username','$password','$email','$phone','$sem','$dept','$session','$regno','$address','$utype','$photo','yes','$vkey','yes')");
		if ($insert) {
			$to = "$email";
